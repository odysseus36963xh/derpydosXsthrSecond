...
/* =========================================================
   LOCAL AUTOSAVE
   =========================================================
   Everything typed into the table is saved automatically to
   this device's localStorage as it's edited (debounced), and
   restored automatically the next time the page is opened.
   This does NOT depend on any server/backend and does NOT
   depend on the user remembering to click "Save Table Data" —
   it's a safety net so nothing is ever lost, even on a phone.

   Media (images/audio/video) is intentionally left out of the
   autosave copy because browsers cap localStorage at a few MB
   total — trying to stuff base64 media in there would quickly
   overflow and could break the autosave for everyone. Media is
   still fully included in the manual "Save Table Data" JSON
   file download, which has no such size cap.
   ========================================================= */
const AUTOSAVE_KEY = "speechTable_autosave_v1";
let autosaveTimer = null;
let autosaveWarned = false;

function buildLightweightSnapshot() {
    const table = document.getElementById("sheet");
    if (!table) return null;

    const headerRow = table.rows[0];
    const colCount = headerRow ? Math.max(headerRow.cells.length - 1, 0) : 26;
    ...
    for (let r = 2; r < table.rows.length; r++) {
        ...
        for (let c = 1; c <= colCount; c++) {
            ...
            } catch (e) { /* keep going, leave this cell blank */ }
            rowData.push(text);
        }
        cells.push(rowData);
    }
    ...
    let practiceData = {};
    try {
        practiceData = window.exportPracticeData ? window.exportPracticeData() : {};
    } catch (e) { /* non-fatal */ }
...
function autosaveTable() {
    try {
        ...
        showAutosaveStatus("Autosaved " + new Date().toLocaleTimeString());
    } catch (err) {
        // Most likely a QuotaExceededError on a very large table.
        console.error("Autosave failed:", err);
        if (!autosaveWarned) {
            autosaveWarned = true;
            showAutosaveStatus("⚠️ Autosave storage full — use Save Table Data to export a file");
        }
    }
}
...
function scheduleAutosave() {
    ...
    autosaveTimer = setTimeout(autosaveTable, 800);
}
...
function restoreAutosave(silent) {
    ...
    try {
        ...
        console.error("Autosave data was corrupted, ignoring it:", e);
        return false;
    }
    ...
    // Clear existing data rows (keep header + language-selector rows).
    ...
    data.cells.forEach((rowArray, r) => {
        ...
        rowArray.forEach((val, c) => {
            ...
            td.dataset.listenCount = "0";
        });
    });

    // Keep the row-add counter in sync with what we just restored.
    if (typeof totalRows !== "undefined") {
        try { totalRows = table.rows.length - 2; } catch (e) { /* ignore */ }
    }

    if (data.practiceData && window.restorePracticeData) {
        try { window.restorePracticeData(data.practiceData); } catch (e) { /* non-fatal */ }
    }

    if (!silent) showAutosaveStatus("Restored autosave from " + (data.savedAt ? new Date(data.savedAt).toLocaleString() : "earlier"));
    return true;
}

// Restore any previous session immediately (the table skeleton built by the
// inline script in index.html has already run by the time this file loads).
restoreAutosave(true);

// Autosave whenever a cell is edited (contenteditable cells fire 'input').
document.getElementById("sheet")?.addEventListener("input", scheduleAutosave);

// Also catch cases where cells are filled programmatically (column upload,
// AI translate, etc.) rather than typed — these don't fire 'input', so poll
// lightly as a backstop, and also save right before the page is closed.
window.addEventListener("beforeunload", autosaveTable);
setInterval(autosaveTable, 15000);
...
// ===============================
async function exportTableData(options) { // <-- Now async
    const includeMedia = !options || options.includeMedia !== false;
    const table = document.getElementById("sheet");
    if (!table) return null;

    // Compute the real column count from the table itself instead of a
    // hardcoded number, so this keeps working even if the table ever
    // grows past its current column layout.
    const headerRow = table.rows[0];
    const colCount = headerRow ? Math.max(headerRow.cells.length - 1, 0) : 26;

    const dataRows = Math.max(table.rows.length - 2, 0);
    const cells = [];
    const languages = [];
    const media = {}; // Stores media data
    ...
    // Process cells and media. Every row/cell access is defensive so that
    // one malformed row can never abort the whole save — worst case that
    // single cell is saved as blank text, but every other row still saves.
    for (let r = 2; r < table.rows.length; r++) {
        const row = table.rows[r];
        const rowData = [];
        ...
        for (let c = 1; c <= colCount; c++) {
            ...
            try {
                ...
                if (includeMedia && cell && cell.dataset && cell.dataset.mediaUrls && cell.dataset.mediaTypes) {
                    ...
                    if (urls.length > 0) {
                        ...
                        for (let i = 0; i < urls.length; i++) {
                            try {
                                ...
                                console.error("Media export failed for cell", r, c, mediaErr);
                            }
                        }
                    }
                }
            } catch (cellErr) {
                // Never let a single bad cell take down the entire export.
                console.error("Cell export failed, saving as blank:", r, c, cellErr);
            }
            rowData.push(text);
        }
        cells.push(rowData);
    ...
    let practiceData = {};
    try {
        practiceData = exportPracticeData ? exportPracticeData() : {};
    } catch (practiceErr) {
        console.error("Practice data export failed:", practiceErr);
    }
...
async function saveTable() {
    ...
    openSaveDialog(defaultName, async (finalName) => { // <-- Make callback async
        const cleanName = finalName.endsWith(".json") ? finalName : finalName + ".json";

        let data = null;
        try {
            data = await exportTableData();
        } catch (err) {
            console.error("Full export (with media) failed, retrying without media:", err);
        }

        // If the media-inclusive export failed for any reason, fall back to a
        // text-only export so the table's actual data is never lost — this is
        // the important part the user is editing, media is a bonus.
        if (!data) {
            try {
                data = await exportTableData({ includeMedia: false });
            } catch (err2) {
        ...
        if (!data) {
            alert("⚠️ Save failed unexpectedly. Please screenshot this and try again — your table data is still safe in the app and has also been autosaved locally on this device.");
            return;
        }

        // Warn if file is huge
        const jsonString = JSON.stringify(data, null, 2);
        const sizeMB = parseFloat((new Blob([jsonString]).size / 1024 / 1024).toFixed(2));
        if (sizeMB > 10) {
            if (!confirm(`Warning: Save file is ${sizeMB}MB. Continue?`)) return;
        }

        downloadJSON(cleanName, data);
        autosaveTable(); // keep the local autosave copy in sync too
        showAutosaveStatus("Saved " + new Date().toLocaleTimeString());
    });
}
// ===============================
...
function uploadTable() {
 ...
 if (data.practiceData) {     
   restorePracticeData(data.practiceData); }  
              ...
              autosaveTable(); // sync the local autosave with what was just loaded
              showAutosaveStatus("Loaded " + new Date().toLocaleTimeString());
