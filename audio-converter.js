...
// -----------------------------------------------------
// EXTRACT WHOLE COLUMN (AUDIO FILES)
// Scans every row of the given column for cells that
// hold a generated audio clip (cell.dataset.audio, set
// by placeAudioInCell during conversion) and pulls them
// all out at once — either as individual downloads or
// bundled into a single .zip.
// -----------------------------------------------------
window.extractColumnAudio = async function (mode) {
  const statusEl = document.getElementById("extractColumnStatus");
  const letterInput = document.getElementById("extractColumnLetter");
  const letter = (letterInput?.value || "").trim().toUpperCase();

  if (!/^[A-Z]+$/.test(letter)) {
    alert("Enter a valid column letter, e.g. B.");
    return;
  }

  const col = colToIndex(letter);
  const usedNames = new Set();
  const found = []; // { blob, filename }

  if (statusEl) statusEl.textContent = "Scanning column " + letter + "...";

  for (let r = 1; r <= totalRows; r++) {
    const cell = getCellByCoords(r, col);
    if (!cell || !cell.dataset.audio) continue;
    ...
    try {
      ...
      filename += (cell.dataset.filename || "").match(/\.[^.]+$/)?.[0] || ".webm";

      found.push({ blob, filename });
    } catch (err) {
      console.error("Could not read audio for row", r, err);
    }
  }

  if (!found.length) {
    if (statusEl) statusEl.textContent = "";
    alert(`No generated audio files found in column ${letter}.`);
    return;
  }

  if (mode === "zip") {
    if (typeof JSZip === "undefined") {
      alert("Zip library didn't load (no internet?) — falling back to individual downloads.");
      mode = "files";
    } else {
      if (statusEl) statusEl.textContent = `Zipping ${found.length} file(s)...`;
      const zip = new JSZip();
      found.forEach(f => zip.file(f.filename, f.blob));
      const zipBlob = await zip.generateAsync({ type: "blob" });
      const stamp = new Date().toISOString().replace(/[:.]/g, "-");
      downloadBlob(zipBlob, `column_${letter}_audio_${stamp}.zip`);
      if (statusEl) statusEl.textContent = `✅ Downloaded ${found.length} file(s) as one .zip.`;
      return;
    }
  }

  if (mode === "files") {
    if (statusEl) statusEl.textContent = `Downloading ${found.length} file(s)...`;
    for (let i = 0; i < found.length; i++) {
      downloadBlob(found[i].blob, found[i].filename);
      // Small stagger so the browser doesn't block a burst of downloads.
      await wait(200);
    }
    if (statusEl) statusEl.textContent = `✅ Downloaded ${found.length} file(s) individually.`;
  }
};
